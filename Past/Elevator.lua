...
local function drawFloors()
  ...
  for i = maxFloor, minFloor, -1 do
    ...
    local x, y = mon.getCursorPos();
    mon.print(text);
    buttonXY[i] = {sx=x, bx=x + text:len()-1, sy=y, by=y};
  end
end
...
while true do
  ...
  local floor = getFloor(x, y);
  if (floor) then
    local negBit = 15;
    if (floor >= 0) then negBit = 1; end
    redstone.setAnalogOutput("right", negBit);
    sleep(0.1);
    redstone.setAnalogOutput("right", math.abs(floor) + 1);
    sleep(0.1);
