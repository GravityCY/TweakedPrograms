...
if (not fs.exists(fileName)) then write("No Such File"); error() end
local song = nbsAPI.load(fileName);
if (not song) then write("Couldn't load Song..."); error() end
song.tempo = forceTempo or song.tempo;
if (song.name == "") then song.name = "No Name"; end
write(string.format("Playing %s (%sbps). %ss", song.name, song.tempo, song.length / 20));
