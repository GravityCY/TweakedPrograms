...
local cancelKey = keys.backspace;
local exitKey = keys.enter;
...
local function getPeripherals(str)
  term.clear();
  term.setCursorPos(1, 1);
  print(("Detecting all %s."):format(str));
  local t = {};
  while true do
    local event, a1 = os.pullEvent();
    if (event == "key") then
      local key = a1;
      if (key == exitKey) then break end
      if (key == cancelKey) then return end
...
local function loadData()
  ...
  redEnableName = data.redEnableName;
  crafterName = data.crafterName;
  dumperAddr = data.dumperName;
  crafterId = data.crafterId;
  overflowNames = data.overflowNames;
...
local function saveData()
  ...
  data.redEnableName = getPeripheral();
  detectUI("Crafter Turtle");
  data.crafterName = getPeripheral();
  detectUI("Dumper Turtle");
  data.dumperName = getPeripheral();
  data.overflowNames = getPeripherals("Overflow Inventories");
  data.interfaceNames = getPeripherals("Interface Inventories");
  data.storageNames = getPeripherals("Storage Inventories");
  write("Enter Crafting Turtle ID: ")
  data.crafterId = tonumber(read());
...
local function setup()
  ...
  enablePeriph = PerUtils.get(redEnableName);
  modemPeriph = peripheral.find("modem");
  crafterTurtle = PerUtils.get(crafterName);
