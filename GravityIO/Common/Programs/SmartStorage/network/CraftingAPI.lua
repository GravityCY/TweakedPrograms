...
-- Every single item needed including subrecipe resources
local function getCraftTotal(recipe, times)
  local t1 = {};
  times = times or 1;
  for _, type in pairs(recipe.resources) do
    local subrecipe = CraftingAPI.get(type);
    if (subrecipe ~= nil) then
      for subtype, total in pairs(getCraftTotal(subrecipe, times)) do
        t1[subtype] = (t1[subtype] or 0) + total;
      end
    else
      t1[type] = (t1[type] or 0) + times;
    end
  end
  return t1;
end

local function newCraft(product, want)
  local recipe = CraftingAPI.get(product);
  if (recipe == nil) then return end
  want = want or recipe.count;
  for type, total in pairs(getCraftTotal(recipe, math.ceil(recipe.count / want))) do
    print(type, total);
  end
end
